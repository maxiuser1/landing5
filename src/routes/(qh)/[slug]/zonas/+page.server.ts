...
import { formatDate } from '$lib/utils/formats/fechas';

export const load = async ({ locals, params }: { locals: App.Locals; params: Record<string, string> }) => {
	const evento: App.Evento = await locals.eventosRepo.getEventoConLocacion(params.slug);
	...
	if (evento && evento.precios) {
		const preventas = evento.precios
			...
			.map((t) => t.descuentos!)
			.flat();
		const preventasOrdenadas = preventas.filter((t) => t.habilitado && t.tipo == 'Pre-Venta').sort((a, b) => a.fechaInicio.localeCompare(b.fechaInicio));
